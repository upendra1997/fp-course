...
-- Implement the traverse function for a Traversable instance for Compose
  traverse f_a_kb (Compose fga) = let fgkb = (f_a_kb <$>) <$> fga
                                      fkgb = sequenceA <$> fgkb
                                      kfgb = sequenceA fkgb
                                  in Compose <$> kfgb
...
-- Implement the (<$>) function for a Functor instance for Product
  (<$>) f' (Product f g) = Product (f' <$> f) (f' <$> g)
    -- error "todo: Course.Traversable (<$>)#instance (Product f g)"

instance (Traversable f, Traversable g) =>
  Traversable (Product f g) where
-- Implement the traverse function for a Traversable instance for Product
  traverse f_a_kb (Product f g) = let f' = traverse f_a_kb f
                                      g' = traverse f_a_kb g
                                      p = Product <$> f' <*> g'
                                  in p
...
-- Implement the traverse function for a Traversable instance for Coproduct
  traverse f_a_kb (InL f) =  InL <$> traverse f_a_kb f
  traverse f_a_kb (InR f) =  InR <$> traverse f_a_kb f
