...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k b
  (<$>) f sa = StateT $ g
               where g s' = let k = runStateT sa s'
                            in f' <$> k
                     f' (a, s) = (f a, s)
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' sa s = case runStateT sa s of
                 (ExactlyOne op) -> op
...
execT ::
  ...
  -> s
  -> k s
execT sta s = snd <$> (runStateT sta s)
...
evalT ::
  ...
  -> s
  -> k a
evalT sta s = fst <$> (runStateT sta s)
...
distinct' ::
  ...
  List a
  -> List a
distinct' ls = eval' (filtering (state' . g) ls) S.empty
                 where g a s = do
                                 let result = P.not (a `S.member` s)
                                 let s' = if a `S.member` s then s else a `S.insert` s
                                 (result, s')
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF = sequence . (<$>) g . distinct'
  where g a = if a > 100 then Empty else Full a
...
instance Applicative (Logger l) where
  ...
  (<*>) ::
    ...
    -> Logger l a
    -> Logger l b
  (<*>) (Logger ls f) (Logger ls' a) = Logger (ls ++ ls') (f a)
    -- error "todo: Course.StateT (<*>)#instance (Logger l)"
...
distinctG ls = runOptionalT $ evalT (filtering (StateT . g) ls) S.empty
                 where g a s = do
                                 let result = P.not (a `S.member` s)
                                 let s' = if a `S.member` s then s else a `S.insert` s
                                 let log'
                                       | a > 100 = log1 (fromString ("aborting > 100: " P.++ show a))
                                       | even a = log1 (fromString ("even number: " P.++ show a))
                                       | otherwise = pure
                                 OptionalT $ if a > 100 then log' Empty else log' $ Full (result, s')
